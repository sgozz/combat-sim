...
/**
 * Knockdown: 2-action Strike that automatically knocks target prone on critical hit.
 * 
 * Requirements:
 * - Knockdown feat
 * - 2 actions remaining
 ...
 * - Costs 2 actions
 * - Makes Strike attack
 * - On critical hit: target becomes prone (automatic, no Athletics check)
 */
export const handlePF2Knockdown = async (
  socket: WebSocket,
  matchId: string,
  match: MatchState,
  player: Player,
  actorCombatant: ReturnType<typeof getCombatantByPlayerId>,
  payload: { type: "pf2_knockdown"; targetId: string }
): Promise<void> => {
  const adapter = getServerAdapter('pf2');
  if (!adapter.pf2) {
    sendMessage(socket, { type: "error", message: "PF2 adapter not available." });
  ...
  if (!hasFeat(attackerCharacter, 'Knockdown')) {
    sendMessage(socket, { type: "error", message: "You do not have the Knockdown feat." });
    return;
  }

  const pf2ActionsRemaining = actorCombatant.actionsRemaining;
  if (pf2ActionsRemaining < 2) {
    sendMessage(socket, { type: "error", message: "Knockdown requires 2 actions." });
    return;
  }
  ...
  let logEntry = `${attackerCharacter.name} uses Knockdown ⚔️⚔️ on ${targetCharacter.name} with ${weapon.name}`;
  if (mapPenalty < 0) {
    logEntry += ` (MAP ${mapPenalty})`;
  }
  if (rangePenalty < 0) {
  ...
  // Apply prone condition ONLY on critical hit
  let finalCombatants = updatedCombatants;
  if (attackRoll.degree === 'critical_success') {
    finalCombatants = updatedCombatants.map(c => {
      if (c.playerId === targetCombatant.playerId && isPF2Combatant(c)) {
        logEntry += `. Target is knocked prone!`;
        return {
          ...c,
